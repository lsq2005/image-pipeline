...
> "Harness 解决的是**流程控制**问题。Prompt 管输入质量，Context 管信息供给，Harness 管执行循环——它像一个独立裁判，在 Agent 每次输出后给出反馈，让 Agent 在循环中不断修正方向。
>
> 这让我联想到 CNN 的训练循环——前向传播产生输出，损失函数独立评估误差，反向传播调整参数。Harness 本质上就是把这种'输出→评估→调整→再输出'的循环用到了 Agent 的行为控制上。
>
> 区别在于 CNN 调整的是数学权重，Harness 调整的是 Agent 的行为策略——用哪个工具、换哪种思路、要不要拆成子任务。"
