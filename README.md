...
1. **为什么是分布式？** 数据量增长到百万级时单机 IO 成为瓶颈。Spark on YARN 将计算分发到数据所在节点，利用数据本地性减少网络传输。

2. **为什么先过滤再去重？** 清晰度过滤 O(n)，自连接去重 O(n²)。先用廉价操作削减 n，减少后续计算量。

3. **为什么 Parquet 而不是 CSV？** 列式存储 + 压缩 + 内置 Schema。查询只涉及某几列时无需扫描全部数据。

4. **阈值怎么定的？** describe() 看分布 → orderBy 看最低分样本 → 人眼判断 → 拍板。不是查表查出来的，是针对数据特性决定的。

5. **如果数据量扩大 1000 倍？** 自连接 O(n²) 不再适用 → 改用 LSH 分桶 + 桶内精确比较；Pandas UDF 替换逐行 UDF。
